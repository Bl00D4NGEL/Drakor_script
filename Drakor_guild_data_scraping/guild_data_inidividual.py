    ...
    for dataThing in rev_data:
        if user == dataThing[2]:
            ...
            if "has removed" in dataThing[0] and 'cLinkType' in dataThing[0]:
                ...
                amount = "???"
                items["Unknown"] = "?"
            #big try block, will only throw exceptions rarely and if so, error message should be clear enough
            try:
            ...
            try:
                ...
                print ex
            try:
                #Try to concat the item statistics into HTML-table strings
                if displayGuildBankInteractions == "true" and (action == "Removed an item from the guild bank" or action == "Removed an item with a link into the guild bank" or action == "Deposited an item into the guild bank" or action == "Deposited an item with a link into the guild bank"):
                    websiteHTML += ("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td</tr>").format(timestamp, user, action, usedItem, amount, items[usedItem])
                elif displayForumInteractions == "true" and action == "Changed/Created something in the guild forum":
                    websiteHTML += ("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td</tr>").format(timestamp, user, action, usedItem, amount, items[usedItem])
                elif displayGoldInteractions == "true" and (action == "Removed gold from the guild coffers" or action == "Removed gold from the guild coffers"):
                    websiteHTML += ("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td</tr>").format(timestamp, user, action, usedItem, amount, items[usedItem])
                elif displayGuildStorageExpansion == "true" and (action == "Expanded Guild Storage with a container" or action == "Expanded Guild Storage with tokens"):
                    websiteHTML += ("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td</tr>").format(timestamp, user, action, usedItem, amount, items[usedItem])
                elif displayRosterChanges == "true" and (action == "Accepted someone to the guild" or action == "Changed something in the Guild roster" or action == "Kicked someone out of the Guild" or action == "Joined the guild" or action == "Joined the guild" or action == "Left the guild" or action == "Rejected an application of someone"):
                    websiteHTML += ("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td</tr>").format(timestamp, user, action, usedItem, amount, items[usedItem])
            except Exception as ex:
                print ex
                print ("{0} has caused an exception when trying to interact with a variable.\nAction: {1}\nTimestamp in the .vlc file: {2}").format(user, action, dataThing[1])
            if action == "Unknown":
                websiteHTML += ("<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td</tr>").format(timestamp, user, "Unknown Action", "Unknown item", "???", "???")
                items[dataThing[0]] = "?"
...
websiteFile = open("DrakorStatisticsIndividual.html", "w")
