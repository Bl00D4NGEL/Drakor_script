...
function CheckInventory(){
	$.ajax("/inventory").success(function(data){
		...
		var duras = data.match(/remaining durability\: (\d+)/gi);
		var output = '| ';
...
function DisenchantSetup(){
	...
	for(var i=0;i<selects.length;i++){
		...
		var select = $(document.createElement("select")).css("font-size", "12px").on("change", function(){
			localStorage.setItem($(this).attr('id'), $(this).val());
		});
		for(var j=0;j<rarities.length;j++){
			var option = $(document.createElement("option")).text(rarities[j]).attr({value:  rarities[j]}).appendTo(select);
